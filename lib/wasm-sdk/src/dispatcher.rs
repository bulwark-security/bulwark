...
impl Dispatcher {
    ...
    fn on_http_request_initial(
        ...
        context_id: u32,
        num_headers: usize,
        readable_body_size: usize,
        end_of_stream: bool,
    ) {
        if let Some(http_stream) = self.http_streams.borrow_mut().get_mut(&context_id) {
            self.active_id.set(context_id);
            http_stream.on_http_request_initial(num_headers, readable_body_size, end_of_stream)
        } else {
            panic!("invalid context_id")
        }
    }
    ...
    fn on_http_request_decision(
        ...
        if let Some(http_stream) = self.http_streams.borrow_mut().get_mut(&context_id) {
            self.active_id.set(context_id);
            http_stream.on_http_request_decision(num_headers, readable_body_size, end_of_stream)
        } else {
            panic!("invalid context_id")
        }
    }

    fn on_http_request_trailers(&self, context_id: u32, num_trailers: usize) {
        if let Some(http_stream) = self.http_streams.borrow_mut().get_mut(&context_id) {
            self.active_id.set(context_id);
    ...
    fn on_http_response_initial(
        ...
        if let Some(http_stream) = self.http_streams.borrow_mut().get_mut(&context_id) {
            self.active_id.set(context_id);
            http_stream.on_http_response_initial(num_headers, readable_body_size, end_of_stream)
        } else {
            panic!("invalid context_id")
        }
    }
    ...
    fn on_http_response_decision(
        ...
        if let Some(http_stream) = self.http_streams.borrow_mut().get_mut(&context_id) {
            self.active_id.set(context_id);
            http_stream.on_http_response_decision(num_headers, readable_body_size, end_of_stream)
        } else {
            panic!("invalid context_id")
        }
    }

    fn on_http_response_trailers(&self, context_id: u32, num_trailers: usize) {
        if let Some(http_stream) = self.http_streams.borrow_mut().get_mut(&context_id) {
            self.active_id.set(context_id);
...
pub extern "C" fn bulwark_on_request_initial(
    ...
    DISPATCHER.with(|dispatcher| {
        dispatcher.on_http_request_initial(
            context_id,
            num_headers,
            readable_body_size,
            end_of_stream,
...
#[no_mangle]
pub extern "C" fn bulwark_on_request_decision(
    context_id: u32,
    num_headers: usize,
    readable_body_size: usize,
    end_of_stream: bool,
    ...
    DISPATCHER.with(|dispatcher| {
        dispatcher.on_http_request_decision(
            context_id,
            num_headers,
            readable_body_size,
            end_of_stream,
...
#[no_mangle]
pub extern "C" fn bulwark_on_request_trailers(context_id: u32, num_trailers: usize) {
    DISPATCHER.with(|dispatcher| dispatcher.on_http_request_trailers(context_id, num_trailers))
}

#[no_mangle]
pub extern "C" fn bulwark_on_response_initial(
    context_id: u32,
    num_headers: usize,
    readable_body_size: usize,
    end_of_stream: bool,
    ...
    DISPATCHER.with(|dispatcher| {
        dispatcher.on_http_response_initial(
            context_id,
            num_headers,
            readable_body_size,
            end_of_stream,
...
#[no_mangle]
pub extern "C" fn bulwark_on_response_decision(
    context_id: u32,
    num_headers: usize,
    readable_body_size: usize,
    end_of_stream: bool,
    ...
    DISPATCHER.with(|dispatcher| {
        dispatcher.on_http_response_decision(
            context_id,
            num_headers,
            readable_body_size,
            end_of_stream,
...
#[no_mangle]
pub extern "C" fn bulwark_on_response_trailers(context_id: u32, num_trailers: usize) {
    DISPATCHER.with(|dispatcher| dispatcher.on_http_response_trailers(context_id, num_trailers))
}

#[no_mangle]
pub extern "C" fn bulwark_on_http_call_response(
    _context_id: u32,
    token_id: u32,
    num_headers: usize,
    body_size: usize,
...
#[no_mangle]
pub extern "C" fn bulwark_on_grpc_receive_initial_metadata(
    _context_id: u32,
    token_id: u32,
    headers: u32,
) {
...
#[no_mangle]
pub extern "C" fn bulwark_on_grpc_receive(_context_id: u32, token_id: u32, response_size: usize) {
    DISPATCHER.with(|dispatcher| dispatcher.on_grpc_receive(token_id, response_size))
}

#[no_mangle]
pub extern "C" fn bulwark_on_grpc_receive_trailing_metadata(
    _context_id: u32,
    token_id: u32,
    trailers: u32,
) {
...
#[no_mangle]
pub extern "C" fn bulwark_on_grpc_close(_context_id: u32, token_id: u32, status_code: u32) {
