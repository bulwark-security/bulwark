...
/// Accesses the Redis connection pool and script registry.
pub struct RedisPool {
    inner: crate::wit::bulwark::plugin::redis::Pool,
}
...
impl RedisPool {
    ...
    pub fn get<K: AsRef<str>>(&self, key: K) -> Result<Option<Vec<u8>>, crate::RemoteStateError> {
    ...
    pub fn get_string<K: AsRef<str>>(
        &self,
        key: K,
    ) -> Result<Option<String>, crate::RemoteStateError> {
        let key: &str = key.as_ref();
    ...
    ///
    /// * `key` - The key name corresponding to the state value.
    pub fn get_i64<K: AsRef<str>>(&self, key: K) -> Result<Option<i64>, crate::RemoteStateError> {
        let key: &str = key.as_ref();
        let bytes = self.inner.get(key)?;
        if let Some(bytes) = bytes {
    ...
    pub fn set_string<K: AsRef<str>, V: AsRef<str>>(
        ...
        Ok(self.inner.set(key, &value.as_bytes().to_vec())?)
    }
    ...
    pub fn set_i64<K: AsRef<str>>(
        ...
        Ok(self
            .inner
            .set(key, &value.to_string().as_bytes().to_vec())?)
    }
    ...
    pub fn del<I: IntoIterator<Item = T>, T: Into<String>>(
        ...
        Ok(self.inner.del(keys.as_slice())?)
    }
    ...
    pub fn incr<K: AsRef<str>>(&self, key: K) -> Result<i64, crate::RemoteStateError> {
        let key: &str = key.as_ref();
        Ok(self.inner.incr(key)?)
    }
    ...
    pub fn incr_by<K: AsRef<str>>(
        &self,
        key: K,
        delta: i64,
    ) -> Result<i64, crate::RemoteStateError> {
        let key: &str = key.as_ref();
        Ok(self.inner.incr_by(key, delta)?)
    }
    ...
    pub fn sadd<K: AsRef<str>, I: IntoIterator<Item = T>, T: Into<String>>(
        ...
        Ok(self.inner.sadd(key, members.as_slice())?)
    }

    pub fn smembers<K: AsRef<str>>(&self, key: K) -> Result<Vec<String>, crate::RemoteStateError> {
        let key: &str = key.as_ref();
        Ok(self.inner.smembers(key)?)
    }
    ...
    pub fn srem<K: AsRef<str>, I: IntoIterator<Item = T>, T: Into<String>>(
        ...
        let key: &str = key.as_ref();
        let members: Vec<String> = members.into_iter().map(|s| s.into()).collect();
        Ok(self.inner.srem(key, members.as_slice())?)
    }
    ...
    pub fn expire<K: AsRef<str>>(&self, key: K, ttl: i64) -> Result<(), crate::RemoteStateError> {
        let key: &str = key.as_ref();
        Ok(self.inner.expire(key, ttl)?)
    }
    ...
    pub fn expire_at<K: AsRef<str>>(
        &self,
        key: K,
        unix_time: i64,
    ) -> Result<(), crate::RemoteStateError> {
        let key: &str = key.as_ref();
        Ok(self.inner.expire_at(key, unix_time)?)
    }
    ...
    pub fn incr_rate_limit<K: AsRef<str>>(
        ...
        let key: &str = key.as_ref();
        Ok(self.inner.incr_rate_limit(key, delta, window)?)
    }
    ...
    ///
    /// * `key` - The key name corresponding to the state counter.
    pub fn check_rate_limit<K: AsRef<str>>(&self, key: K) -> Result<Rate, crate::RemoteStateError> {
        let key: &str = key.as_ref();
        Ok(self.inner.check_rate_limit(key)?)
    }
    ...
    pub fn incr_breaker<K: AsRef<str>>(
        ...
        Ok(self
            .inner
            .incr_breaker(key, success_delta, failure_delta, window)?)
    }
    ...
    /// * `key` - The key name corresponding to the state counter.
    #[inline]
    pub fn check_breaker<K: AsRef<str>>(&self, key: K) -> Result<Breaker, crate::RemoteStateError> {
        let key: &str = key.as_ref();
        Ok(self.inner.check_breaker(key)?)
    }
}

pub fn acquire() -> Result<RedisPool, crate::RemoteStateError> {
    Ok(RedisPool {
        inner: crate::wit::bulwark::plugin::redis::Pool::acquire()?,
    })
}
